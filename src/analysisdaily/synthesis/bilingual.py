...
def _chunk_text(text: str, maxlen: int = 3000) -> list[str]:
    """按空行（段落/条目边界）把长文切成 <=maxlen 的块，避免单次翻译被输出上限截断。"""
    paras = text.split("\n\n")
    chunks: list[str] = []
    cur = ""
    for p in paras:
...
def translate_to_zh(settings: Settings, markdown_en: str) -> str:
    ...
    try:
        ...
        parts: list[str] = []
        for ch in _chunk_text(markdown_en):
            try:
                parts.append(chat_completion(
                    settings,
                    [{"role": "system", "content": _SYS}, {"role": "user", "content": ch}],
                    temperature=0.2, max_tokens=5000,
                ).strip())
            except Exception:  # noqa: BLE001
                parts.append(ch)  # 该块翻译失败则保留英文
        return "\n\n".join(parts)
    except Exception:  # noqa: BLE001
        return markdown_en  # 整体失败则保留英文版

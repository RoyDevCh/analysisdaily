...
def _resolve(settings: Settings) -> tuple[str, str, str]:
    """返回 (base_url, api_key, model)。支持 openrouter 专用 key。"""
    if settings.llm_provider == "openrouter":
        key = settings.openrouter_api_key or settings.llm_api_key
        return "https://openrouter.ai/api/v1", key, settings.llm_model
...
def chat_completion(settings: Settings, messages: list[dict], temperature: float = 0.1, max_tokens: int = 900) -> str:
    base, key, model = _resolve(settings)
